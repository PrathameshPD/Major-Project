...
for feature in categorical_features:
    ...
    data_clean.loc[:, feature] = label_encoders[feature].fit_transform(data_clean[feature]) #Use .loc[] to explicitly modify the DataFrame. This avoids ambiguity.
#Why?
#.loc[] ensures you are working on the correct DataFrame and not on a potentially unintended copy.
...
# Create a DataFrame for the input data with the correct column names
input_data = pd.DataFrame(
    [[age, gender_encoded, tumor_type_encoded, tumor_grade_encoded, tumor_location_encoded, treatment_encoded, treatment_outcome_encoded]],
    columns=features  # Ensure the columns match the features used during training
)

# Perform the prediction
recurrence_time = model.predict(input_data)
